...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // -- Scintillator --
  {
    ...
    fScintiLog  = new G4LogicalVolume(
        solid, G4Material::GetMaterial("Scintillator_PVT"), "Scintillator");
    fScintiPhys = new G4PVPlacement(
        nullptr, G4ThreeVector(0,0,0), fScintiLog, "Scintillator",
        fWorldLog, false, 0, true);
  ...
  // -- Optical grease --
  {
    ...
    fGlueLog    = new G4LogicalVolume(
        solid, G4Material::GetMaterial("OpticalGrease"), "Glue");
    fGluePhys   = new G4PVPlacement(
        nullptr, G4ThreeVector(0,0,glueZ), fGlueLog, "Glue",
        fWorldLog, false, 0, true);
  ...
  // -- PMT glass window --
  {
    ...
    fPMTWindowPhys = new G4PVPlacement(
        nullptr, G4ThreeVector(0,0,windowZ), fPMTWindowLog, "PMTWindow",
        fWorldLog, false, 0, true);
  ...
  // -- PMT body (vacuum) --
  {
    ...
    fPMTPhys    = new G4PVPlacement(
        nullptr, G4ThreeVector(0,0,pmtZ), fPMTLog, "PMT",
        fWorldLog, false, 0, true);
  ...
  auto* outerTube = new G4Tubs("AlOuterTube",
      0, fPMTRadius + t, tubeHZ, 0, 360.*deg);

  // Union: box + tube (tube offset in Z)
  auto* outerUnion = new G4UnionSolid("AlOuterUnion",
      outerBox, outerTube,
      nullptr, G4ThreeVector(0, 0, tubeCentreZ));
  ...
  auto* innerCyl = new G4Tubs("AlInnerCyl",
      0, fPMTRadius, cylHZ, 0, 360.*deg);

  // Subtract scintillator cavity
  auto* sub1 = new G4SubtractionSolid("AlSub1",
      outerUnion, innerBox,
      nullptr, G4ThreeVector(0, 0, 0));

  // Subtract PMT/glue/window cylinder cavity
  auto* alShellSolid = new G4SubtractionSolid("AlFoil",
      sub1, innerCyl,
      nullptr, G4ThreeVector(0, 0, cylCentreZ));

  fAlLog  = new G4LogicalVolume(
      alShellSolid, G4Material::GetMaterial("Aluminum_Refl"), "AlFoil");
  fAlPhys = new G4PVPlacement(
      nullptr, G4ThreeVector(0,0,0), fAlLog, "AlFoil",
      fWorldLog, false, 0, true);
